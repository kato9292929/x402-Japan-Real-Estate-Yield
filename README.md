...
> AI-powered real estate investment yield analysis for Japan, served as an x402-enabled API.
>
> 日本の不動産投資利回りをエリア別に AI 分析する、x402 対応データ API。
...
no API keys, pay per request in USDC on the Base network.
...
- Next.js 15 / React 19
- `x402-next` for per-route payment (`withX402`)
- `@anthropic-ai/sdk` (Claude) for analysis
- `wagmi` / `viem` / `@rainbow-me/rainbowkit` for wallet connection
...
| `WALLET_ADDRESS`                     | Address that receives x402 payments                |
...
都度課金されます。API キーは不要で、Base ネットワークの USDC により
リクエスト単位で支払います。
...
- Next.js 15 / React 19
- ルート単位の課金に `x402-next`（`withX402`）
- 分析エンジンに `@anthropic-ai/sdk`（Claude）
- ウォレット接続に `wagmi` / `viem` / `@rainbow-me/rainbowkit`
